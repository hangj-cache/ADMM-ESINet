...
if __name__ == '__main__':
    ...
    # len_train, len_validate = len(train), len(validate)
    # print("len_train: ", len_train, "\tlen_validate:", len_validate)
    ...
    ###############################################################################
    # mask
    # V_dir = args.simulation_data + "validation\\various conditions\V.mat"
    # V_data = loadmat(V_dir)
    # mtx = V_data['V']
    # dense_mtx = mtx.toarray()
    # V = torch.tensor(dense_mtx).to("cuda").float()
    ...
    # simulation_L_path = "Data\\DST_CedNet\\train\\various conditions\L.mat"
    # L = torch.tensor(loadmat(simulation_L_path)['Gain']).to("cuda").float()

    lambda1 = 10   #original ：10
    lambda2 = 1e5  #original :150
    # lambda2 = 1  #original :150
    # lambda3 = 1e-6
    # lambda = 200
    # lambda_SNRs = 10
    delta = 0.01  #原来0.01
    ...
    for epoch in range(0, args.num_epoch + 1):
        ...
        # for batch_idx, data in tqdm(enumerate(train_loader),desc='Training',unit='file'):
            ratio = 1
            ...
            # s_real_trans = torch.bmm(s_real,TBFs_tp)
